...
class TcpInjEventContainer(KwEventContainer):
    """
    Used by ingest script to process TCP packet inject events 
    """
    def __init__(self, packet_class, i_col_name, b_col_name, container_type):
        KwEventContainer.__init__(self, packet_class, i_col_name, b_col_name)

        # Create blocked IP dictionary  { [ IP : time ] }
        # For testing.
        #self.blocked_ip = { (1,2,3,4) : time.time() }
        self.blocked_ip = {}
        self.blocked_info = {}           # Holds event info
        if trafcap.options.mongo:
            self.db = trafcap.mongoSetup()
        return
    ...
    def updateDb(self):
        ...
        for key in self.blocked_info:
            # Update mongo if needed
            a_info = self.blocked_info[key]
            ...
            if ((a_info[pc.i_csldw] == True) and
                ...
                if trafcap.options.mongo:
                    ...
                    blocked_info_doc = pc.buildInfoDoc(a_info)

                    try:
                        # If _id field exists in dictionary, upsert is done
                        if a_info[pc.i_id] != None:
                            blocked_info_doc['_id'] = a_info[pc.i_id]

                        _id=self.db[self.info_collection].save(blocked_info_doc)
                        ...
                        trafcap.logException(e, a_info=a_info,
                                            blocked_info_doc=blocked_info_doc)
        ...
        for key in keys_to_pop:
            try:
                expired_info = self.blocked_info.pop(key)
            except KeyError:
                continue
            if not trafcap.options.quiet:
                print("\rEi:", expired_info)
    ...
    def expireIp(self):
        pc = self.pc
        keys_to_pop = []
        for key in self.blocked_ip:
            if (trafcap.current_time - self.blocked_ip[key][1]) > 0:
                 keys_to_pop.append(key)

        for key in keys_to_pop:
            try:
                expire_time = self.blocked_ip.pop(key)
            except KeyError:
                continue
            if not trafcap.options.quiet:
                print("\rEi:", key, expire_time)
