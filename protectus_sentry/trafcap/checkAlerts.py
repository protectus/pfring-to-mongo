#!/usr/bin/python
#
# Copywrite (c) 2013 PROTECTUS,LLC.  All Rights Reserved.
#
...
class Alert(object):
    ...
    def messageBody(self):
        returnable = ""
        for key,item in self.key_value.items():
...
def email_alert(alert, alert_state, mailto, **alert_doc):
    ...
    smtp_tuple_list = []
    try:
        smtp_server_list.append(sentry_config_doc["smtp_server"])
    except KeyError:
        # Lookup MX record if no SMTP server is specified
        try:
            domain = mailto.split('@')[-1]
            smtp_servers = dns.resolver.query(domain, 'MX')
            for a_server in smtp_servers:   # for debug
                smtp_server = a_server.exchange.to_text().rstrip('.')
                smtp_pref = a_server.preference
                smtp_tuple_list.append((smtp_server, smtp_pref))

            # List of tuples: (server_name as string, preference as integer)
            sorted_smtp_tuple_list = sorted(smtp_tuple_list, key=lambda tup:tup[1])
            for tup in sorted_smtp_tuple_list:
                smtp_server_list.append(tup[0])

        except Exception as e:
            raise LookupError("Unable to find MX record for " + mailto + " : " + str(e) + ". Aborting alert.")
    ...
    # Send mail and retry if needed until server_list is exhausted
    for smtp_server in smtp_server_list:
        try:
            mailserver = smtplib.SMTP(smtp_server, smtp_port)
            mailserver.sendmail(msg['from'], [mailto], msg.as_string())

            # If we make it this far, then the email was sent
            log_msg = 'Notification sent to: ' + mailto + ' for: ' + alert.one_liner
            success = True
            break

        except smtplib.SMTPException as e:
            log_msg += "Attemped email to " + smtp_server + ": SMTPException: " + str(e) + '. '
            pass
        
        except Exception as e:
            raise LookupError("Unable to send email: " + str(e) + ". Aborting alert.")
...
def main():
    ...
    def thresholded_ids_check(state, match_doc, message, threshold, timeout, **unknown_args):
        ...
        new_last_id = db.ids_eventInfo.find_one(projection={"_id":1}, sort=[('_id', -1)])['_id']
        ...
        if "last_id" not in state:
            ...
            log(syslog.LOG_DEBUG, "No last_id found in thresholded_ids_check state")
            return []
        ...
        log(syslog.LOG_DEBUG, "thresholded_ids_check(): match_doc = " + str(match_doc))
    ...
    def simple_ids_check(state, match_doc, message, rate_limit=None, **unknown_args):
        ...
        for event in events:
            source = IPAddress(event["src"])
            src_port = str(event.get('src_port', 'none'))
            dest = IPAddress(event["dst"])
            dst_port = str(event.get('dst_port', 'none'))
    ...
    if options.verbose:
        for key in state: print('key: ', key, ' : ', state[key])
        ...
        for alert_doc in alert_docs:
            ...
            for alert in alerts:
                ...
                try:
                    ...
                    success, log_msg = alert_command(alert, alert_state, **alert_doc)
                    if success: alerts_sent += 1
                    if len(log_msg) >= 1: log(log_msg)
                except Exception as e:
