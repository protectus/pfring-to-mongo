...
#  Javier Kohen (jkohen)
#
# Modified for Python3 on Sentry9 - PFG - March 2018
...
class Header(PacketBuffer,ProtocolLayer):
    ...
    # Not sure about this...
    packet_printable = str(filter(lambda c: c not in string.whitespace, string.printable)) + ' '
...
class Ethernet(Header):
    ...
    def as_eth_addr(anArray):
        ...
        return '' + functools.reduce(lambda x, y: x+':'+y, tmp_list)
...
# tcp.set_th_off(5)
# 
# ip.set_ip_src('192.168.1.19')
# ip.set_ip_dst('206.180.172.130')
...
# tcp.set_th_seq(1000+1)
# tcp.set_th_ack(0)         # 0 by default
# ip.contains(tcp)
# s.sendto(ip.get_packet(), ('206.180.172.130', 80))
